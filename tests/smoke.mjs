...
const unitPersonnelSource = fs.readFileSync(path.join(root, "src/ui/render/unitPersonnel.js"), "utf8");
...
assert.doesNotMatch(appSource, /function collectUnitPersonnel\(/);
assert.match(unitPersonnelSource, /function collectUnitPersonnel\(/);
assert.match(unitPersonnelSource, /renderUnitRoster\(state, indexes, selectedUnitId\)/);
...
assert.equal(sameA.gameVersion, "0.4.3.22");
...
// Existing schema-10 saves migrate forward without regenerating or moving the career.
{
  ...
  assert.equal(payload.worldState.gameVersion, "0.4.3.22");
