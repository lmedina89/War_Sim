...
export function createPlayerCareer(store, registries, input) {
  ...
  const rank = registries.ranks.get(branch.startingRankId);
  const startingRole = registries.roles.get(branch.startingRoleId);
  const unitId = state.world.careerStartUnitByBranchId[branch.id];
  const unit = state.entities.units[unitId];
  if (!unit) throw new Error(`No starting unit is configured for ${branch.name}.`);

  const billet = Object.values(state.entities.billets).find(candidate => {
    if (candidate.unitId !== unitId || candidate.status !== "vacant") return false;
    return registries.billets.get(candidate.definitionId).roleId === startingRole.id;
  });
  ...
  store.mutate(draft => {
    ...
    const serviceRecordId = createEntityId(draft, "service");
    const loadoutId = createEntityId(draft, "loadout");
    ...
    draft.entities.people[personId] = {
      ...
      career: { enlistmentDate: draft.world.date, experience: 0, prestige: 0 },
      condition: { health: 100, morale: 75, fatigue: 0, readiness: 70, status: "active" },
      traitIds: [],
      loadoutId,
      serviceRecordId,
      simulationTier: 0
    ...
    draft.entities.serviceRecords[serviceRecordId] = {
      id: serviceRecordId, schemaVersion: 1, personId,
      serviceStatus: "active", entryDate: draft.world.date, separationDate: null
    };

    draft.entities.equipmentInstances[equipmentInstanceId] = {
      id: equipmentInstanceId, schemaVersion: 1,
      definitionId: "weapon_service_rifle", ownerPersonId: personId,
      condition: 100, upgradeIds: []
    };

    draft.entities.loadouts[loadoutId] = {
      id: loadoutId, schemaVersion: 2, ownerPersonId: personId,
      slots: { primaryWeaponInstanceId: equipmentInstanceId }
    };

    draft.entities.billets[billet.id].assignedPersonId = personId;
    draft.entities.billets[billet.id].status = "filled";

    draft.entities.assignmentRecords[assignmentId] = {
      id: assignmentId, schemaVersion: 2, personId, unitId,
      billetId: billet.id, startDate: draft.world.date,
    ...
    draft.entities.careerEvents[enlistmentEventId] = {
      ...
      references: { branchId: branch.id, rankId: rank.id, unitId, billetId: billet.id }
    };
    ...
    for (const npc of Object.values(draft.entities.people)) {
      ...
      draft.entities.relationshipRecords[relationshipId] = {
        id: relationshipId, schemaVersion: 1,
        personAId: personId, personBId: npc.id,
        familiarity: 5, trust: 0, respect: 0, bond: 0,
        relationshipType: "squadmate", lastInteractionDate: draft.world.date
      };
    }

    noticeId = recordNotification(draft, {
      personId, type: "career_started", title: "Career Started",
      message: `${firstName} ${lastName} enlisted in the ${branch.name} as ${rank.abbreviation}.`,
      priority: "normal",
      references: { branchId: branch.id, rankId: rank.id, unitId, billetId: billet.id }
    });

    recordAction(draft, {
      actorPersonId: personId, commandType: "create_player_career",
  ...
  return commandResult({
    ...
    data: { personId, billetId: billet.id },
