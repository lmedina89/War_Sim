...
function renderOrganization(state, indexes, personId) {
  ...
  const assignmentFormation=document.createElement("div");assignmentFormation.className="assignment-formation";
  if(formationView?.formationInsigniaId)assignmentFormation.append(createNamedInsignia(formationView.formationInsigniaId,{title:formationView.formationName}),metricBlock("FORMATION",formationView.formationName));
  els.assignmentCard.replaceChildren(assignmentFormation,statLine("Duty Position", assignment.billetName), statLine("Assigned Since", assignment.assignmentStartDate), statLine("Chain", assignment.chain.map(x => x.name).join(" › ")));
  ...
  els.organizationBrowser.replaceChildren(); const command=document.createElement("div");command.className="unit-command-header";const commandTop=document.createElement("div");commandTop.className="unit-command-identity";const commandCopy=document.createElement("div");const label=document.createElement("span");label.className="command-label";label.textContent=documentProfile("unit_status").classification;const unitTitle=document.createElement("strong");unitTitle.textContent=current.name.toUpperCase();const echelon=document.createElement("span");echelon.textContent=`${current.echelon} · ${current.branch}`;commandCopy.append(label,unitTitle,echelon);if(current.formationInsigniaId)commandTop.append(createNamedInsignia(current.formationInsigniaId,{title:current.formationName}));commandTop.append(commandCopy);const metrics=document.createElement("div");metrics.className="unit-command-metrics";metrics.append(metricBlock("PERS",`${aggregate.assigned}/${aggregate.authorized}`),metricBlock("VAC",aggregate.vacancies),metricBlock("RDY",`${current.readiness}%`),metricBlock("MORALE",`${current.morale}%`));command.append(commandTop,metrics); els.organizationBrowser.append(command);
