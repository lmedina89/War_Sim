...
function showPersonProfile(personId) {
  ...
  if(!career.qualifications.length&&!career.awards.length){const empty=document.createElement("p");empty.className="empty-state military-empty";empty.textContent="NO QUALIFICATIONS OR AWARDS RECORDED";recordSection.appendChild(empty);} else {for(const q of career.qualifications){const detail=[q.result?.toUpperCase(),q.score!=null&&q.maxScore!=null?`${q.score}/${q.maxScore}`:null,q.expiresDate?`EXP ${q.expiresDate}`:null].filter(Boolean).join(" · ");recordSection.appendChild(statLine(q.name,detail||q.completedDate));}for(const a of career.awards)recordSection.appendChild(statLine(a.name,a.earnedDate));}
