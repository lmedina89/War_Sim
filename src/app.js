...
function showPersonProfile(personId) {
  ...
  if(!career.education.length&&!career.qualifications.length&&!career.awards.length){const empty=document.createElement("p");empty.className="empty-state military-empty";empty.textContent="NO MILITARY EDUCATION, QUALIFICATIONS, OR AWARDS RECORDED";recordSection.appendChild(empty);} else {for(const e of career.education)recordSection.appendChild(statLine(e.name,`GRADUATED · ${e.completedDate??"—"}`));for(const q of career.qualifications){const detail=[q.result?.toUpperCase(),q.score!=null&&q.maxScore!=null?`${q.score}/${q.maxScore}`:null,q.expiresDate?`EXP ${q.expiresDate}`:null].filter(Boolean).join(" · ");recordSection.appendChild(statLine(q.name,detail||q.completedDate));}for(const a of career.awards)recordSection.appendChild(statLine(a.name,a.earnedDate));}
...
function render() {
  ...
  if(!career.education.length&&!career.qualifications.length&&!career.awards.length){const empty=document.createElement("p");empty.className="empty-state military-empty";empty.textContent="NO MILITARY EDUCATION, QUALIFICATIONS, OR AWARDS RECORDED";els.schoolsAwards.appendChild(empty);} else {
    ...
    addGroup("Military Education",career.education,item=>{const row=document.createElement("div");row.className="record-strip";row.append(statusStamp("filled"),metricBlock("SCHOOL",item.name),metricBlock("STATUS",item.status.toUpperCase()),metricBlock("COMPLETED",item.completedDate??"—"));return row;});
    addGroup("Qualifications",career.qualifications,item=>{const row=document.createElement("div");row.className="record-strip";const result=[item.result?.toUpperCase(),item.score!=null&&item.maxScore!=null?`${item.score}/${item.maxScore}`:null].filter(Boolean).join(" · ")||"QUALIFIED";row.append(statusStamp("filled"),metricBlock("QUALIFICATION",item.name),metricBlock("RATING",result),metricBlock(item.expiresDate?"EXPIRES":"COMPLETED",item.expiresDate??item.completedDate));return row;});
    addGroup("Badges & Tabs",career.awards.filter(item=>["badge","tab"].includes(item.category)),item=>{const row=document.createElement("div");row.className="record-strip";row.append(statusStamp("filled"),metricBlock(item.category.toUpperCase(),item.name),metricBlock("EARNED",item.earnedDate));return row;});
    addGroup("Ribbons, Medals & Decorations",career.awards.filter(item=>!["badge","tab"].includes(item.category)),item=>{const row=document.createElement("div");row.className="record-strip";row.append(statusStamp("filled"),metricBlock(item.category.toUpperCase(),item.name),metricBlock("EARNED",item.earnedDate));return row;});
  }
