const REQUIRED_STORES = [
  "people", "units", "unitSlots", "serviceRecords", "loadouts", "equipmentInstances", "careerEvents", "assignmentRecords", "promotionRecords",
  "awardRecords", "qualificationRecords", "deploymentRecords", "casualtyRecords", "memorialRecords", "relationshipRecords", "notificationRecords", "actionRecords"
];
function requireRef(errors, store, id, context) { if (id != null && !store[id]) errors.push(`${context}: missing reference ${id}.`); }
export function validateWorldState(state, registries) {
  ...
  if (state.schemaVersion !== 3) errors.push(`Unsupported world-state schemaVersion: ${state.schemaVersion}`);
  if (!state.world?.date) errors.push("Missing world date.");
  if (!state.world?.clock || !Number.isInteger(state.world.clock.elapsedDays)) errors.push("Missing or invalid simulation clock.");
  if (!Number.isInteger(state.world?.nextEntitySequence) || state.world.nextEntitySequence < 1) errors.push("Invalid nextEntitySequence.");
  if (!Number.isInteger(state.world?.seed) || !Number.isInteger(state.world?.rngState)) errors.push("Invalid deterministic RNG state.");
  for (const name of REQUIRED_STORES) if (!state.entities?.[name] || typeof state.entities[name] !== "object") errors.push(`Missing entity store: ${name}`);
  ...
  for (const person of Object.values(e.people)) {
    ...
    if (!registries.simulationTiers.values().some(t => t.tier === person.simulationTier)) errors.push(`${person.id}: invalid simulationTier.`);
  ...
  for (const record of Object.values(e.assignmentRecords)) { requireRef(errors, e.people, record.personId, `${record.id}.personId`); requireRef(errors, e.units, record.unitId, `${record.id}.unitId`); if (!registries.roles.has(record.roleId)) errors.push(`${record.id}: invalid roleId.`); }
  for (const record of Object.values(e.promotionRecords)) { requireRef(errors, e.people, record.personId, `${record.id}.personId`); if (!registries.ranks.has(record.rankId)) errors.push(`${record.id}: invalid rankId.`); if (!registries.ranks.has(record.previousRankId)) errors.push(`${record.id}: invalid previousRankId.`); }
  ...
  for (const record of Object.values(e.qualificationRecords)) { requireRef(errors, e.people, record.personId, `${record.id}.personId`); if (!registries.schools.has(record.schoolId)) errors.push(`${record.id}: invalid schoolId.`); if (!registries.qualifications.has(record.qualificationId)) errors.push(`${record.id}: invalid qualificationId.`); }
  for (const record of Object.values(e.casualtyRecords)) requireRef(errors, e.people, record.personId, `${record.id}.personId`);
  for (const record of Object.values(e.memorialRecords)) { requireRef(errors, e.people, record.personId, `${record.id}.personId`); requireRef(errors, e.casualtyRecords, record.casualtyRecordId, `${record.id}.casualtyRecordId`); }
  for (const record of Object.values(e.relationshipRecords)) { requireRef(errors, e.people, record.personAId, `${record.id}.personAId`); requireRef(errors, e.people, record.personBId, `${record.id}.personBId`); if (record.personAId === record.personBId) errors.push(`${record.id}: relationship cannot reference the same person twice.`); }
  for (const record of Object.values(e.notificationRecords)) requireRef(errors, e.people, record.personId, `${record.id}.personId`);
  for (const record of Object.values(e.actionRecords)) requireRef(errors, e.people, record.actorPersonId, `${record.id}.actorPersonId`);
